...
class DebugStateMachine:
    def __init__(self, robot, scene, control, pos = None) -> None:
        self.robot = robot
        self.scene = scene
        self.control = control
        self.simulation = self.scene.simulation
        ...
        # targets are [x, y, z] coordinates
        self.target = pos if pos else self.scene.get_target_pos_euler()[0]
        #th to position ee infront of target
        self.y_th = 0.5
        self.thetas = None
        self.curr_state_target = self.robot.ee_home
        self.curr_final_target = self.scene.get_target_pos_euler()[0]
        ...
        self.start_pos = robot.ee_home 
        self.start_config = robot.home 
        self.start_orientation = orientations[0]
        self.target_orientation = orientations[0]
    ...
    def next_state(self):
        ...
        if self.curr_state == States.INIT:
            ...
            self.steps_thetas = []
            #set final target
            self.curr_final_target = self.target
            #set state target and configuration
            self.curr_state_target = self.start_pos
            self.curr_state_configuration = self.start_config
            #next state
        ...
        # primary condition to move to the next step is the distance to the current state target
            ...
            if self.curr_state == States.HOME:
                self.simulation.data.set_mocap_pos("target",  self.curr_final_target+[0, 0, 0.1])
                if not self.look_at_target:
                    # create the next states, positions
                    self.steps_positions = [] #empty list
                    curr_ee_pos = self.robot.get_ee_position()
